...
"""
FINAL CHRONOS V1-V6 vs STATISTICAL / ML BENCHMARK
===========================================
...
RESULTS_DIR = (
    BASE_DIR
    / "results_final_chronos_v1_v6_vs_statistical"
)
...
def build_v6_features(
    ...
    # Only reference covariates can be carried across small
    # market-calendar gaps. Stock-derived variables are never filled.
    d[ref_cols] = (
        d[ref_cols]
        .ffill(limit=3)
    )
...
def make_v6_context(
    ...
    if len(d) < 10:
        raise ValueError(
            f"{ticker}: only "
            f"{len(d)} aligned V6 observations"
        )
...
def predict_chronos_v6(
    ...
    context = make_v6_context(
        history,
        references,
        ticker,
    )

    # First use the low-level covariate API to avoid frequency inference.
    try:

        output = (
            try_low_level_predict_quantiles(
                pipeline,
                context,
                horizon,
                V6_COVARIATES,
        ...
        return extract_quantiles_from_output(
            output,
            horizon,
        )
    ...
    # Compatibility fallback for Chronos releases exposing predict_df.
    ...
    for model_name, model_path in (
        ...
        if model_name == "Chronos-LoRA-V6":
            try:
                test_ticker = TICKERS[0]
                test_stock = load_stock(test_ticker)
                test_history = test_stock.iloc[:min(len(test_stock), 700)].copy()
                _ = predict_chronos_v6(
                    pipeline,
                    test_history,
                    references,
                    test_ticker,
                ...
                print("V6 covariate preflight: PASS")
            except Exception as exc:
                del pipeline
                gc.collect()
                if torch.cuda.is_available():
